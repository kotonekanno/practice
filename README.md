...
このリポジトリは、学習で作成した小規模な練習コードをまとめたものです。  
成果物としてではなく、学習過程の記録や参照のために保管しています。  
各ファイルは基本的に単体で完結しており、コマンドラインから直接実行できるシンプルなプログラムです。  

## 注目のプログラム

小規模な作品となっているプログラムを別のリポジトリにまとめています。  
プログラムを実行する場合は、こちらのリポジトリをクローンするのがおすすめです。  
→　[Small Works](https://github.com/kotonekanno/small-works)  
<br>
※以下のリンクは、このリポジトリ内のファイルを指しています
